...
If the user doesn't have a Slack app, share [SLACK_SETUP.md](SLACK_SETUP.md). Direct them to the **Fast path** at the top — it takes about 2 minutes:

1. Create app from manifest at [api.slack.com/apps](https://api.slack.com/apps) using `slack-app-manifest.json` from the project root
2. Generate an App-Level Token (`xapp-...`) under Basic Information → App-Level Tokens
3. Install to workspace and copy the Bot Token (`xoxb-...`)
4. Run `npx tsx scripts/setup-slack.ts` — it handles `.env` and `data/env/env` automatically

Wait for the user to confirm the script ran successfully.
